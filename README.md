...
For a render that is already stuck rather than an environment that is misconfigured, `php artisan render:status` is the one to reach for — it reports the queue depth and gives each recent attempt a verdict.
